...
from api_credentials import RAPID_API__KEY, RAPID_API__HOST  # Import API credentials
...
def fetch_and_update_game_stats(games_csv, games_stats_csv):
    ...
    if len(diff_game_ids) == 0:
        ...
        return final_df.to_csv("include/raw_datasets/games_stats.csv", index=False)


games_df = 'include/raw_datasets/games.csv'
games_stats_df = 'include/raw_datasets/games_stats.csv'
